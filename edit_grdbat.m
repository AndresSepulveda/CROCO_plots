...
%
%--------------------------------------------------------------------------
%function editbat_V2c(cmin,cmax,bathymetryfile, coastlinefile) %#ok<FNDEF>
function edit_grdbat(cmin,cmax,bathymetryfile) %#ok<FNDEF>
...
if (nargin < 3)
    [fn,pth]=uigetfile('*.*','Select NETCDF bathymetry file ...');
    if (~fn),
        return,
    else
...
%    [fn,pth]=uigetfile([pth '*.*'],'Select coastline file (Optional) ...');
%    if (fn),
...
[xx,yy,zz] = load_bathymetry(bathymetryfile);
...
nc{'lon'}(:)=xdata;
nc{'lat'}(:)=ydata;
nc{'topo'}(:,:)=get(handles.image,'cdata');
close(nc)
...
%--------------------------------------------------------------------------
% When a change occurr in Detph Edit, the color must be actualized
%--------------------------------------------------------------------------
...
% Loading NETDCF file.
%--------------------------------------------------------------------------
function [xx,yy,zz] = load_bathymetry(archivo_batimetria)
nc=netcdf(archivo_batimetria);
xx=nc{'lon_rho'}(1,:)-360.;
yy=nc{'lat_rho'}(:,1);
zz=nc{'h'}(:,:);
close(nc);
...
% Ploting
%--------------------------------------------------------------------------
function handles = DrawMap(xx,yy,zz,costax,costay,...
                           rango_colores,bathymetryfile)
global handles;
handles.image = imagesc(xx,yy,zz,'Parent',handles.axis.graph.control);
handles.axis.colorbar.control = colorbar;
eje = axis;
hold on;
handles.coast = plot(costax,costay);
hold off;
axis equal;
axis(eje);
