function lonlatmld_v1(fname,Yorig,posi,posf,step,tname)
...
clear MLD
MLD=zeros(size(temp,2),size(temp,3));
...
MLD(i,j) = ra_mld(flipud(squeeze(salt(:,i,j))),flipud(squeeze(temp(:,i,j))),flipud(squeeze(z(:,i,j))),0.3);
    end
end
...
h1=m_pcolor(lon,lat,squeeze(MLD));
