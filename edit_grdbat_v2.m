...
function edit_grdbat(cmin,cmax,bathymetryfile,coastlinefile)
...
if (nargin < 3)
    [fn,pth]=uigetfile('*.nc','Select NETCDF bathymetry file ...');
    if (~fn),
        return,
    else
...
[xx,yy,zz,mask] = load_bathymetry(bathymetryfile);
...
% Loading NETDCF file.
%--------------------------------------------------------------------------
function [xx,yy,zz,mask] = load_bathymetry(archivo_batimetria)
...
% Ploting
%--------------------------------------------------------------------------
function DrawMap(xx,yy,zz,mask,costax,costay,rango_colores,bathymetryfile)
global handles;
mask(mask==0)=NaN;
handles.image = imagesc(xx,yy,zz.*mask,'Parent',handles.axis.graph.control);
